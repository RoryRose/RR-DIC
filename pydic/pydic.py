...
#Corrected and Rotations added by Phani Karamched, University of Oxford
...
(ii) smooth the displacement field thanks to the chosen method (raw, linear, spline,etc.)"""
     ...
     def compute_strain_field_DA(self):
          ...
          self.rotation_xy = self.disp_x.copy(); self.rotation_xy.fill(np.NAN)
          ...
          for i in range(self.size_x):
            for j in range(self.size_y):
                 ...
                 self.rotation_xy[i,j] = .5*(dv_dx - du_dy)
...
def pick_area_of_interest(PreliminaryImage):
    ...
    image = cv2.putText(PreliminaryImage, "Left click + drag area and press 'c' continue", (50,50), cv2.FONT_HERSHEY_SIMPLEX, 1,(255,255,255),4)
